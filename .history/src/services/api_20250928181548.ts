...
export const fetchUserProfile = async (): Promise<User> => {
  try {
    const response = await api.get<{success: boolean; data: User}>("/userAuth/profile");
...
export const updateUserProfile = async (
  ...
  try {
    const response = await api.put<{success: boolean; data: User}>("/userAuth/profile", userData);
    if (response.data.success && response.data.data) {
      return response.data.data;
    }
