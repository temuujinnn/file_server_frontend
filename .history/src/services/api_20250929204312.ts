...
export const downloadProduct = async (productId: string): Promise<void> => {
  ...
  try {
    ...
    const ticketResponse = await fetch(`${baseUrl}/user/game/download?id=${productId}`, {
      method: "GET",
      headers: {
    ...
    if (!ticketResponse.ok) {
      throw new Error(`Failed to get download ticket (${ticketResponse.status})`);
    }
    ...
    const downloadUrl = `${baseUrl}/user/game/download_link?id=${ticketId}`;
