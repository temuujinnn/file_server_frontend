  ...
  useEffect(() => {
    const loadTags = async () => {
      try {
        ...
        // Auto-expand selected main tag category and clear additional tag selection
        if (selectedMainTag) {
          const categoryToExpand = organizedCategories.find(
            (cat) => cat.name === selectedMainTag
          );
          if (categoryToExpand) {
            setExpandedCategories(new Set([categoryToExpand.id]));
            // Clear selected additional tag when main tag is selected
            onTagSelect(null);
          }
        }
  ...
  const handleMainTagClick = (mainTag: string) => {
    if (onMainTagSelect) {
      ...
      if (selectedMainTag === mainTag) {
        ...
        const categoryToExpand = categories.find((cat) => cat.name === mainTag);
        if (categoryToExpand) {
          setExpandedCategories(new Set([categoryToExpand.id]));
        }
      }
      ...
      <div className="space-y-3">
        ...
        {filteredCategories.map((category) => (
          <div key={category.id} className="space-y-1">
            {/* Category Header */}
            <button
              onClick={() => handleMainTagClick(category.name)}
