...
const TagSidebar: React.FC<TagSidebarProps> = ({
  ...
  const handleMainTagClick = useCallback(
    (mainTag: string) => {
      ...
      if (selectedMainTag === mainTag) {
        // Deselecting current main tag - smooth transition
        setExpandedCategories(new Set());
        // Small delay to allow smooth collapse animation
        setTimeout(() => {
  ...
  const handleClearFilter = useCallback(() => {
    // Smooth transition - collapse first, then clear
    setExpandedCategories(new Set());
    setTimeout(() => {
      onTagSelect(null);
      if (onMainTagSelect) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 border border-gray-200 dark:border-gray-700 shadow-sm transition-shadow duration-200 hover:shadow-md">
      ...
      <div className="space-y-2">
        ...
        <button
          ...
          className={`w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 ease-out transform hover:scale-[1.01] active:scale-[0.99] ${
        ...
        {filteredCategories.map((category) => (
          <div key={category.id} className="space-y-1">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 ease-out transform hover:scale-[1.01] active:scale-[0.99] ${
                ...
                <div className="ml-6 mt-2 space-y-1 border-l-2 border-blue-200 dark:border-blue-600 pl-4">
                  {category.tags.map((tag, index) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-md text-xs font-medium transition-all duration-300 ease-out transform hover:scale-[1.02] ${
                        selectedTag &&
